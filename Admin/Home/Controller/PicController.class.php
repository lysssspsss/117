...
use Think\Auth;
class PicController extends AuthController {
...
$hot=I('post.hot','','strip_tags'); ######安全模式过滤获取标题
...
//↓↓↓↓↓↓↓↓↓↓↓利用AR数据添加方法
$Article=D("Home/pic");       // 实例化Data数据对象
$Article->title=$title;
...
$Article=D("Home/pic")->find($id);          ###实例化Data数据对象
...
$title=I('post.title','','strip_tags');     ######安全模式过滤获取标题
$hot=I('post.hot','','strip_tags');     ######安全模式过滤获取标题
$hits=I('post.hits','','strip_tags');     ######安全模式过滤获取标题
$picurl=I('post.path','','strip_tags');     ######安全模式过滤获取标题
$ClassID=I('post.ClassID','','strip_tags'); ######安全模式过滤获取标题
$directory1=substr($ClassID,0,4);    ######文章一级栏目
$directory2=substr($ClassID,0,8);    ######文章二级栏目
...
cz_sorting("cz_pic","1",$_GET['Point'],$_GET['id'],$_GET['sid'],$config);
...
$Article=D("Home/pic_class")->find($id);
...
cz_class_sorting("cz_pic_class","0",$_GET['Point'],$_GET['id'],$_GET['sid'],$_GET['numberid'],$config);
echo "<script>self.location=document.referrer;</script>";
exit();	
}
