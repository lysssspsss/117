 ...
 */
    ...
    public function plug_ad_list(){
    	...
    	$plug_ad_list=D('plug_ad')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order('plug_ad_order')->relation(true)->select();
 ...
 */
    public function plug_ad_runadd(){
    	if (!IS_AJAX){
    		...
    		$upload->rootPath  =     './Public/gg/'; // 设置附件上传根目录
    		$upload->savePath  =     ''; // 设置附件上传（子）目录
    		$upload->saveRule  =     'time';
    		$info   =   $upload->upload();
    		
    		if($info) {
    			$img_url='/gg/'.$info[file0][savepath].$info[file0][savename];//如果上传成功则完成路径拼接
			...
			$sl_data=array(
					...
					'plug_ad_js'=>I('plug_ad_js','','htmlspecialchars'),
 ...
 */
    public function plug_adtype_del(){
    	M('plug_adtype')->where(array('plug_adtype_id'=>I('plug_adtype_id')))->delete();//删除广告位
    	M('plug_ad')->where(array('plug_ad_adtypeid'=>I('plug_adtype_id')))->delete();//删除该广告位所有广告
