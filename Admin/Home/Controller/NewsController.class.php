...
$list = $Article->field(array('id','title','time','view','cid','sort'))->where($search)->order("id desc")->limit($Page->limit)->select();
#######读取第一条排序
$Start = $Article->field(array('sort'))->where($search)->order("id desc,time desc")->limit(1)->select();
#######读取后一条排序
$End   = $Article->field(array('sort'))->where($search)->order("id asc,time desc")->limit(1)->select();
$this->assign('Startid',$Start[0]['sort']);
$this->assign('Endid',$End[0]['sort']);
...
session(news_category,$ClassID);
//↓↓↓↓↓↓↓↓↓↓↓利用AR数据添加方法
$Article=D("Home/news");       // 实例化Data数据对象
$Article->title=$title;
...
$Article=D("Home/news")->find($id);          ###实例化Data数据对象
$this->assign('info',$Article);   ######赋值Id
$this->display();	
}elseif($_POST['Action']=='Edit'){
$Id=I('post.Id',0,'intval');                ######安全模式过滤获取标签
$title=I('post.title','','strip_tags');     ######安全模式过滤获取标题
$ID=I('post.ID','','strip_tags'); ######安全模式过滤获取标题
$content=$_POST['content'];          ###内容
$Article=D("Home/news");          ###实例化Data数据对象
...
cz_sorting("cz_news","1",$_GET['Point'],$_GET['id'],$_GET['sid'],$config);
...
$Article=D("Home/news_category")->find($id);
...
cz_class_sorting("cz_news_category","0",$_GET['Point'],$_GET['id'],$_GET['sid'],$_GET['numberid'],$config);
echo "<script>self.location=document.referrer;</script>";
exit();	
}
