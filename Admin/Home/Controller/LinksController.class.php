...
$list = $Article->field(array('id','title','url','begtime','directory4','sorting'))->where($search)->order("id desc")->limit($Page->limit)->select();
...
session(links_url_class,$ClassID);
$Article=D("Home/links_url");
$Article->title=$title;
$Article->url=$url;
...
$Article=D("Home/links_url")->find($id);
...
cz_sorting("cz_links_url","1",$_GET['Point'],$_GET['id'],$_GET['sid'],$config);
...
cz_class_sorting("cz_links_url_class","0",$_GET['Point'],$_GET['id'],$_GET['sid'],$_GET['numberid'],$config);
echo "<script>self.location=document.referrer;</script>";
exit();	
}
//↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑以上执行信息的排序功能
...
$Article=D("Home/links_url_class");
...
$id=I('get.Id',0,'intval');
$Article=D("Home/links_url_class")->find($id);
$this->assign('info',$Article);
$this->display();
}
//↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑以上执行数据目录修改页面
